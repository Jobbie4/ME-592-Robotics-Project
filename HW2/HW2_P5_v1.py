...
ax.scatter(x, y, normalized_depth, c=rgb.reshape(-1, 3) / 255.0, s=1)
...
#%%
#This section you shouldn't run. Seems to just smooth everything over way too much into a black sheet

"""
import scipy.ndimage as ndimage
smoothed_depth = ndimage.gaussian_filter(depth, sigma=2)

depth_normalized = (smoothed_depth - np.min(smoothed_depth)) / (np.max(smoothed_depth) - np.min(smoothed_depth))
...
x, y = np.meshgrid(np.arange(w), np.arange(h))
points = np.stack((x, y, depth_normalized), axis=-1).reshape(-1, 3)
